...
String RegistrationNumber, RegistrationRoot;
//---------------------------------------------------------------------------
...
// Закройте соединение перед выполнением нового запроса
Napr->Close();
Napr->SQL->Clear();

// Сформируйте SQL-запрос с правильными ключевыми словами
Napr->SQL->Add("USE Practic_work");
Napr->SQL->Add("SELECT * FROM  Subjects order by subjects "); // Имя предмета из таблицы Subjects
Napr->Open();

// Итерация по результатам и добавление уникальных значений в ComboBox
for(int i = 0; i < Napr->RecordCount; i++) {
    bool pool = true;
    ...
    // Добавьте предмет в список, если его нет в ComboBox
    if (pool) {
		Dissipl->Items->Add(Napr->FieldByName("subjects")->AsString);
    }
...
int p = MessageDlg("Вы действительно хотите удалить данную Дисциплину?", mtConfirmation, TMsgDlgButtons() << mbOK << mbCancel, 0);
		if(p == mrOk)
		{
           ...
           ADOQuery5->SQL->Clear();
		   ADOQuery5->SQL->Add("SELECT * FROM Dist ");
		   ADOQuery5->SQL->Add("JOIN Form_Of_Control ON Dist.Form_Of_Control = Form_Of_Control.id ");
		   ADOQuery5->SQL->Add("JOIN Speciality ON Dist.Speciality_Id = Speciality.id ");
		   ADOQuery5->SQL->Add("JOIN Subjects ON Dist.Name_Dist = Subjects.id ");
		   ADOQuery5->SQL->Add("Order by subjects ");
...
// Передаем значения переменных
	Form7->RegistrationNumber = RegistrationNumber;
	Form7->RegistrationRoot = RegistrationRoot;
...
// Передаем значения переменных
	Form5->RegistrationNumber = RegistrationNumber;
	Form5->RegistrationRoot = RegistrationRoot;
...
// Передаем значения переменных
	Form6->RegistrationNumber = RegistrationNumber;
	Form6->RegistrationRoot = RegistrationRoot;
...
// Передаем значения переменных
	Form8->RegistrationNumber = RegistrationNumber;
	Form8->RegistrationRoot = RegistrationRoot;
...
{
	 TForma10 *Forma10 = new TForma10(this);

// Передаем значения переменных
	Forma10->RegistrationNumber = RegistrationNumber;
	Forma10->RegistrationRoot = RegistrationRoot;
...
{
	// Скрыть основную форму при запуске
this->Hide();

TRegistrationForm *RegistrationForm = new TRegistrationForm(this);
bool loginSuccessful = false;

while (!loginSuccessful && RegistrationForm->ShowModal() == mrOk)
{
    // Получить номер из поля Edit1 формы регистрации
    RegistrationNumber = RegistrationForm->Edit1->Text;
    RegistrationRoot = RegistrationForm->ComboBox1->Text;

	// Проверить учетные данные
	if ((RegistrationRoot == "Админ" && RegistrationNumber == "123") || RegistrationRoot == "Гость")
    {
        // Данные верны, устанавливаем флаг успешного входа
        loginSuccessful = true;
        this->Show(); // Показать основную форму
    }
    else
    {
        // Данные неверны, показываем сообщение и остаемся на форме регистрации
        ShowMessage("Пароль неверен. Попробуйте снова.");
    }
}

// Если форма регистрации была закрыта через крестик или Cancel, завершить приложение
if (!loginSuccessful)
{
...
{
    delete RegistrationForm; // Удаляем форму регистрации после успешного входа
}
...
int activeIndex = PageControl1->ActivePageIndex;
	...
	if (RegistrationRoot == "Гость" && (activeIndex == 1 || activeIndex == 2))
	{
