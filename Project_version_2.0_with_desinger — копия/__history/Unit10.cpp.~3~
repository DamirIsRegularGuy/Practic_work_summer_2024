...
void __fastcall TForm10::Button1Click(TObject *Sender)
{
  if(Edit1->Text!="" && Edit2->Text!="")
   ...
   {
	 ShowMessage("ƒл€ добавлени€ событи€, надо ввести хот€ бы дату и им€ соответствующего событи€");
   }
...
void __fastcall TForm10::N1Click(TObject *Sender)
{
  ...
  {
	ShowMessage("—обытие еще не наступило.");
  }
  else if (currentTime > InputTime)
  {
	ShowMessage("—обытие прошло.");
  }
  else if(currentTime == InputTime)
  {
	ShowMessage("—егодн€ день событи€.");
  }
...
{
		int result = MessageDlg("¬ы действительно хотите удалить эти данные, после удалени€ их восстановление будет невозможным", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, 0);
	if (result == mrYes)
	{
	ADOQuery1->Close();
	ADOQuery1->SQL->Clear();
...
String ser;
	 if(Key==32)
	 {
		 ...
		 mot=i;
		 piir="";
		 ShowMessage("ќтлично ћы учли количество уведомлений, теперь если хотите установить определенное врем€ дл€ уведомлени€ - продолжайте вводить в данное поле ваше врем€ с определенным форматом. ƒл€ установлени€ времени по умолчанию наберите 77:77:77.");
		 Edit5->Clear();
		 //Edit4->SetFocus();
	 ...
	 if(Key==13)
	 {
		 ...
		 {
			Edit5->Clear();
			ShowMessage("«начени€ по умолчанию будут установлены в зависимости от ƒаты уведомлени€.");
			//piir="";
		 }
		 ...
		 {
			 ShowMessage("¬ы привысили допустимое значение уведомлений, насто€тельно прошу дальнейший ввод чего-либо.");
			 j=0;
			 Edit5->EditMask = "";
			 Edit5->ReadOnly = true;
			 Edit5->Clear();
