...
{
ADOQuery11->Close();
ADOQuery11->SQL->Text = "";  // Очистите запрос
ADOQuery11->SQL->Add("USE Practic_work");
ADOQuery11->SQL->Add("SELECT * FROM Rasp");
ADOQuery11->Open();

// Перейдите к последней записи в наборе данных
ADOQuery11->Last(); // Переходит к последней записи

// Извлеките номер недели из последней записи
int lastWeekNumber = ADOQuery11->FieldByName("week")->AsInteger;
String needyear = ADOQuery11->FieldByName("year")->AsString;
...
// Добавьте запрос
ADOQuery11->SQL->Add("USE Practic_work");
ADOQuery11->SQL->Add("SELECT DISTINCT Rasp.*, Kabin.room, Prepods.*, Subjects.*, Groupe.*");
ADOQuery11->SQL->Add("FROM Rasp");
ADOQuery11->SQL->Add("INNER JOIN Kabin ON Rasp.id_kabin = Kabin.id");
ADOQuery11->SQL->Add("INNER JOIN Subjects ON Rasp.id_pred = Subjects.id");
ADOQuery11->SQL->Add("INNER JOIN Prepods ON Rasp.id_prep = Prepods.id");
ADOQuery11->SQL->Add("INNER JOIN Groupe ON Rasp.id_gr = Groupe.id");
ADOQuery11->SQL->Add("INNER JOIN Speciality ON Speciality.id = Groupe.spec");
ADOQuery11->SQL->Add("WHERE Prepods.id = :TeacherID");
ADOQuery11->SQL->Add("AND Rasp.week = :WeekNumber");
ADOQuery11->SQL->Add("AND Rasp.year = :Year");
ADOQuery11->SQL->Add("ORDER BY Rasp.data");

// Установите параметры
ADOQuery11->Parameters->ParamByName("TeacherID")->Value = selectedTeacherId;
ADOQuery11->Parameters->ParamByName("WeekNumber")->Value = lastWeekNumber;
ADOQuery11->Parameters->ParamByName("Year")->Value = needyear;

// Откройте запрос
ADOQuery11->Open();

   // Настройте и покажите отчет
...
void __fastcall TForm5::Button4Click(TObject *Sender)
{
	...
	Form11->ADOQuery1->SQL->Add("use Practic_work");
	Form11->ADOQuery1->SQL->Add("select * from Rasp");
	Form11->ADOQuery1->Open();

	// Перейдите к последней записи в наборе данных
	Form11->ADOQuery1->Last(); // Переходит к последней записи

	// Извлеките номер недели из последней записи
	int lastWeekNumber = Form11->ADOQuery1->FieldByName("week")->AsInteger;
	String needyear = Form11->ADOQuery1->FieldByName("year")->AsString;


// Создайте основной запрос, используя номер недели
   String query = "use Practic_work "
	...
	Form11->ADOQuery1->Close();
	Form11->ADOQuery1->SQL->Clear();
	Form11->ADOQuery1->SQL->Add(query);
